...
//		   Program continues to run until the user inputs 0 and will ask the user again for input when inputted a non-integer input.
...
int main(){
	...
	//Error checking if the input is an integer and reprompting until it gets an integer
	while(!(cin >> input)){
		cin.clear();
	...
	while(input != 0){ //Continues to run until the input is 0
		...
		//Asking for new input, this input will be checked by the will loop
		cout << "Enter an interstate number: ";
		
		//Error checking again if the new input is an integer
		while(!(cin >> input)){
			cin.clear();
			cin.ignore(numeric_limits<streamsize>::max(),'\n');
			cout << "Invalid input, try again: ";
